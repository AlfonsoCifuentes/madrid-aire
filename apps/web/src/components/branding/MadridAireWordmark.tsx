...
const madridSizes: Record<NonNullable<MadridAireWordmarkProps["size"]>, string> = {
  ...
  landing: "text-[clamp(2.8rem,11vw,12rem)] 2xl:text-[clamp(2.8rem,8.8vw,8.8rem)]",
...
const aireSizes: Record<NonNullable<MadridAireWordmarkProps["size"]>, string> = {
  ...
  landing: "text-[clamp(2.55rem,10.1vw,10.8rem)] 2xl:text-[clamp(2.55rem,8.2vw,8.2rem)]",
...
export function MadridAireWordmark({
  ...
  return (
    <div className={joinClasses("inline-flex max-w-full min-w-0 items-center", wrapperSizes[size], className)}>
      {showFlag ? (
      ...
      <div className="inline-flex min-w-0 items-center gap-[0.08em] leading-none">
        <span
